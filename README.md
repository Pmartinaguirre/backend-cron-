...
- **`/ganador-semanal`** — corre 1 vez por semana, poco después de que cierra la semana futbolera (martes 00:00 Chile). Por cada grupo, suma los diamantes que ganó cada miembro DURANTE esa semana (solo contando a quien ya era miembro) y guarda al que más sumó como "Ganador semanal" (medalla 🏅 en la tabla de posiciones del grupo, ver `grupo_ganadores_semanales`). Si ya existe un registro para ese grupo+semana, no hace nada (así no importa si corre más de una vez). Además, a cada grupo que SÍ calcula (no a los `yaCalculado`), le manda un mail de recap a todos sus miembros: la tabla de posiciones de la semana que cerró, el ganador+medalla, el anuncio de la semana nueva en juego con su rango de fechas, y los partidos para pronosticar esa semana (botón "Ir a pronosticar partidos") — mejor esfuerzo, si Resend falla solo se loguea, nunca rompe el cálculo del premio. **Sugerencia de horario: 1 vez por semana, el martes a la hora que prefieras (ej. 9:00 hora Chile) — si tu cron no te deja elegir "una vez por semana", correrlo TODOS los días a esa misma hora es seguro: por la idempotencia de arriba, solo el primer día después de cada martes hace algo real, el resto de los días esa semana no hace nada.** `?semana=N` permite recalcular una semana puntual a mano. `?reenviar=1` fuerza el reenvío del mail de recap para una semana que YA estaba calculada (`yaCalculado: true`) — no vuelve a tocar el cálculo del ganador ni el insert, solo reenvía el mail con el ganador que ya está guardado; útil si el mail no salió la primera vez (ej. el deploy con esta función llegó después de que esa semana ya se había calculado a mano).

- **`/aguante-resolver`** — cron semanal (mismo horario que `/ganador-semanal`). Por cada grupo con Aguante activo, resuelve la semana recién cerrada: al que no eligió equipo a tiempo, o cuyo equipo elegido perdió, le quita una vida (empate no mata); a las 2 vidas perdidas, queda eliminado. **Si este cron no está agregado en cron-job.org, nadie pierde vidas nunca** — a diferencia de `/resolver`/`/ganador-semanal`, este NO se agregó a la lista original de cron jobs cuando se construyó El Aguante; revisá que esté.
- **`/baby-resolver`** — cron (mismo horario que `/resolver`, cada 5-15 min). Paga diamantes a quien acertó el ganador/empate de cada partido de Baby ya con marcador oficial. **Igual que `/aguante-resolver`, si no está agregado en cron-job.org, Baby no paga ni acumula diamantes nunca** — revisá que esté.
...
| `/ganador-semanal` | 1 vez por semana, martes (hora Chile — ver nota arriba) | GET |
| `/aguante-resolver` | 1 vez por semana, mismo horario que `/ganador-semanal` | GET |
| `/baby-resolver` | Cada 5-15 minutos, mismo horario que `/resolver` | GET |
